#!/usr/bin/env node
/*
 * normalize-quotes.js — deterministic pass that makes a document's quotation marks
 * and apostrophes consistently STRAIGHT or CURLY. A rewrite tends to emit straight
 * marks regardless of what the surrounding document uses; run this afterward to make
 * them consistent. Guide-agnostic: it takes a target, not a style guide.
 *
 * Usage: node scripts/normalize-quotes.js <file.md> --quotes straight|curly [--write]
 *
 *   straight -> "  '   (curly marks straightened)
 *   curly    -> “ ” ‘ ’ (straight marks educated)
 *
 ...
 *   - hyphens are never rewritten (this pass is quotes/apostrophes only)
 *
 * Education runs on the whole prose line with inline-code spans masked, so a quote
 * next to `code` gets the right direction, and code is restored verbatim afterward
 * (offset-stable because education is a 1:1 character swap).
 *
...
function transformLine(line, quotes) {
  const spans = [];
  const re = /`+[^`]*`+/g;
  let m;
  ...
  let out = quotes === 'straight' ? straighten(masked) : educate(masked); // 1:1, offset-stable
  for (const [s, e] of spans) out = out.slice(0, s) + line.slice(s, e) + out.slice(e);
  return out;
}

/** Normalize marks in `text` to `quotes` ('straight'|'curly'), skipping frontmatter and code. */
function normalize(text, quotes) {
  ...
  for (; i < lines.length; i += 1) {
    ...
    out.push(transformLine(line, quotes));
  }
  return out.join('\n');
}
...
if (require.main === module) {
  ...
  const qIdx = args.indexOf('--quotes');
  const quotes = qIdx >= 0 ? args[qIdx + 1] : null;
  const file = args.find((a) => !a.startsWith('--') && a !== quotes);
  if (!file || !['straight', 'curly'].includes(quotes)) {
    console.error('usage: normalize-quotes.js <file> --quotes straight|curly [--write]');
    process.exit(2);
  }
  const result = normalize(fs.readFileSync(file, 'utf8'), quotes);
  if (write) {
